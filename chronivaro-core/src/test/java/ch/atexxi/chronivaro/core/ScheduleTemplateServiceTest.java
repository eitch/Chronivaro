...
public class ScheduleTemplateServiceTest {
	...
	public void shouldManageScheduleTemplates() {
		...
		// 1. Create
		CreateScheduleTemplateService.CreateScheduleTemplateArgument createArg = new CreateScheduleTemplateService.CreateScheduleTemplateArgument();
		createArg.id = "test-template";
		...
		ServiceResult result = serviceHandler.doService(cert, new CreateScheduleTemplateService(), createArg);
		assertTrue(result.isOk());

		// 2. Verify
		try (StrolchTransaction tx = runtimeMock.openUserTx(cert, true)) {
			Resource template = tx.getResourceBy(TYPE_EMPLOYMENT_SCHEDULE_TEMPLATE, "test-template", true);
			assertEquals("Test Template", template.getName());
			assertEquals(480, (int) template.getInteger("dailyTargetMinutesMonday"));
		}

		// 3. Update
		UpdateScheduleTemplateService.UpdateScheduleTemplateArgument updateArg = new UpdateScheduleTemplateService.UpdateScheduleTemplateArgument();
		updateArg.id = "test-template";
		...
		result = serviceHandler.doService(cert, new UpdateScheduleTemplateService(), updateArg);
		assertTrue(result.isOk());
		...
		// 5. Remove
		result = serviceHandler.doService(cert, new RemoveScheduleTemplateService(), new StringArgument("test-template"));
		assertTrue(result.isOk());

		// 6. Verify Removal
		try (StrolchTransaction tx = runtimeMock.openUserTx(cert, true)) {
			List<Resource> templates = tx.streamResources(TYPE_EMPLOYMENT_SCHEDULE_TEMPLATE).filter(t -> t.getId().equals("test-template")).toList();
