...
public class ApprovePeriodService extends AbstractService<StringArgument, ServiceResult> {
	...
	protected ServiceResult internalDoService(StringArgument arg) throws Exception {
		...
		try (StrolchTransaction tx = openArgOrUserTx(arg)) {
			...
			if (!currentState.equals(STATE_SUBMITTED)) {
				throw new IllegalStateException("Period is not in state SUBMITTED!");
			}

			period.setString(PARAM_STATE, STATE_APPROVED);
			tx.update(period);

			ChronivaroAuditHelper.audit(tx, TYPE_TIME_PERIOD, period.getId(), AUDIT_ACTION_APPROVE,
					"Approved time period " + period.getId() + " for employee " + period.getRelationId(PARAM_EMPLOYEE));
