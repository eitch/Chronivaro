	...
	protected ServiceResult internalDoService(UpdateAbsenceArgument arg) throws Exception {
		...
		try (StrolchTransaction tx = openArgOrUserTx(arg)) {
			...
			boolean isAdminOrHr = tx.getPrivilegeContext().hasRole(ROLE_HR) || tx
					.getPrivilegeContext()
					.hasRole(ROLE_ADMIN) || tx.getPrivilegeContext().hasRole(ROLE_ADMINISTRATOR) || tx
					.getPrivilegeContext()
					.hasRole(ROLE_STROLCH_ADMIN) || tx.getPrivilegeContext().hasRole(ROLE_PRIVILEGE_ADMIN);
			...
			String auditDesc = currentState.equals(STATE_APPROVED) ?
					"Corrected approved absence " + absence.getName() + " for employee " + employee.getString(
							PARAM_PERSONAL_NUMBER) :
					"Updated absence " + absence.getName() + " for employee " + employee.getString(
							PARAM_PERSONAL_NUMBER);
			...
			if (currentState.equals(STATE_APPROVED)) {
				...
				List<Resource> existingVacEntries = tx
				...
				if (VacationHelper.isVacationAbsence(tx, absence)) {
					...
					if (totalMinutes > 0) {
						VacationHelper.assertSufficientVacationBalance(tx, employee.getId(), totalMinutes,
								absence.getDate(PARAM_START));

						Resource entry = tx.getResourceTemplate(TYPE_VACATION_ACCOUNT_ENTRY, true);
						entry.setName("Vacation Usage for absence " + absence.getName());
						...
						entry.setString(PARAM_COMMENT, "Vacation usage for corrected absence " + absence.getName());
						...
						ChronivaroAuditHelper.audit(tx, TYPE_VACATION_ACCOUNT_ENTRY, entry.getId(), AUDIT_ACTION_CREATE,
								format("Created vacation usage entry for corrected absence {0} ({1} minutes)",
